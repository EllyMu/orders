...
def test_prefers_earlier_batches():
    ...
    tomorrow_batch = Batch('shipping-002', 'PRETTY_CHAIR', 20, eta=date.today() + timedelta(days=1))
    week_batch = Batch('shipping-003', 'PRETTY_CHAIR', 20, eta=date.today() + timedelta(days=7))
    line = OrderLine('chairs-001', 'PRETTY_CHAIR', 10)
...
def test_returns_allocated_batch_ref():
    """
    arrange: создать ОДИН батча и товарную позицию;
    act: запросить сервис allocate
...
def test_raises_out_of_stock_exception_if_cannot_allocate():
    ...
    act: разместить в батче больший  объем;
    ...
    with pytest.raises(OutOfStock):
        allocate(line, [batch]) == batch.reference
